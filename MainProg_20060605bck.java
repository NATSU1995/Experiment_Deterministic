       ...
       for (int t = 0; t <= T - 2; t++) {
         ...
         if (cplex.solve()) {
           for (int i = 0; i <= N; i++) {
             ...
             if(debug) debugOut.println("supply "+(t+1)+"_"+i+":\t"+supply[t + 1][i]);
           }
         }
        cplex.clearModel();
...
for(int ite =1;ite <=ITE;ite++){
       ...
       double KStar =0;
       for (int t = T - 2; t >= 0; t--) {
         updateQandU(myNetwork,cplex,t,q[t+1],u[t+1],supply[t],q[t],u[t]);
       }
         ...
         for(int ite2=1;ite2<=ITE2;ite2++){
           ...
           if(snNetwork.getObjfcn() < - 0.1){
             for(int i=0;i<=N;i++){
               ...
               if(debug) debugOut.println("supply "+(t)+"_"+i+":\t"+supply[t][i]);
             }
 ...
 for(int ite =1;ite <=ITE;ite++){
        ...
        for (int t = 0; t <= T - 3; t++) {
          ...
          for (int i = 0; i <= N; i++) {
            ...
            if (G2_x1 <= G2_x2) {
              int stat = binarySearch(myNetwork, t, N, q[t], u[t], qqq, uuu);
              for (int i = 0; i <= N; i++) {
                ArcInfo arc = (ArcInfo) myNetwork.getFirstInArcOf("N" + ( t * (N + 1) + i));
                supply[t ][i ] = 0;
                while (arc != null) {
...
// get the cost of the last stage
  ...
  public int binarySearch(Network myNetwork,int t, int N, double[][] q,double[][]u,double[][] qq,double[][] uu){
    ...
    if(debug) debugOut.println("G1_x1="+G1_x1+",G2_x1="+G2_x1+",G1_x2="+G1_x2+",G2_x2="+G2_x2+",G1_xm="+G1_xm+",G2_xm="+G2_xm);
    if(Math.abs(G1_x2-G2_x2) <=0.1){
      if(debug) debugOut.println("G1_x2==G2_x2, find optimal solution:");
      for(int i=0;i<=N;i++){
        ArcInfo arc = (ArcInfo) myNetwork.getFirstInArcOf("N" +(t * (N + 1) + i));
        while(arc != null){
          arc.pre_flow = arc.flow ;
          if(debug) debugOut.print(arc.getKey()+"\t"+arc.pre_flow+"\t");
          arc = (ArcInfo) myNetwork.getNextInArcFrom(arc.getKey());
        }
        if(debug) debugOut.println();
      }
      ...
      if(debug) debugOut.println("G1_xm==G2_xm,find optimal solution:");
      for(int i=0;i<=N;i++){
        ArcInfo arc = (ArcInfo) myNetwork.getFirstInArcOf("N" +(t * (N + 1) + i));
        while(arc != null){
      ...
      if((G1_x1- G2_x1)*(G1_xm- G2_xm) > 0 && (G2_x2- G1_x2) *(G2_xm- G1_xm) <0){
        ...
        return binarySearch(myNetwork,t, N, q,u,qq,uu);
      }
        ...
        if(debug) debugOut.println("Error in binary search");
        return 2;
      }
...
public void updateQandU(Network myNetwork,IloCplex cplex,int t,double qq[][],double uu[][],double sp[],double q[][],double u[][]){
 try{
    if(debug) debugOut.println("t=:\t"+t);
    int N = myNetwork.N;
    int numArc = 0;
   ...
   if(debug) debugOut.println("error at updating q and u"+e);
 }
  }
